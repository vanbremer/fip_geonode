...
try:
    from fip_geonode.settings import GEONODE_APPS
except:
...
def start_geoserver(options):
    ...
    from fip_geonode.settings import OGC_SERVER
...
def _reset():
    sh("rm -rf fip_geonode/development.db")
    sh("rm -rf fip_geonode/uploaded/*")
    _install_data_dir()
